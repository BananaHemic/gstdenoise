...
spectral_median(float* x,int n)
{
    ...
    if(n%2==0)
    {
      ...
      float val = ((tmp[n/2] + tmp[n/2 - 1]) / 2.f);
	  free(tmp);
	  return val;
    }
    else
               ...
               int* peaks_count, int samp_rate)
{
  ...
  float *fft_magnitude_db = (float*)malloc(sizeof(float) * (fft_size_2 + 1));
									 ...
									 float* whitening_window_count, float max_decay_rate)
{
	...
	float *whitened_spectrum = (float*)malloc(sizeof(float) * N);
