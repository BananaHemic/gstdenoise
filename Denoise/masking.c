              ...
              fftwf_plan* forward_at)
{
  ...
  float *sinewave = (float*)malloc(sizeof(float) * (2 * fft_size_2));
  //float window[2*fft_size_2];
  float *window = (float*)malloc(sizeof(float) * (2*fft_size_2));
  //float fft_p2_at[fft_size_2+1];
  float *fft_p2_at = (float*)malloc(sizeof(float) * (fft_size_2+1));
  //float fft_magnitude_at[fft_size_2+1];
  float *fft_magnitude_at = (float*)malloc(sizeof(float) * (fft_size_2+1));
  //float fft_phase_at[fft_size_2+1];
  float *fft_phase_at = (float*)malloc(sizeof(float) * (fft_size_2+1));
  //float fft_p2_at_dbspl[fft_size_2+1];
  float *fft_p2_at_dbspl = (float*)malloc(sizeof(float) * (fft_size_2+1));
  ...
  free(window);
  free(fft_p2_at);
  free(fft_magnitude_at);
  free(fft_phase_at);
  free(fft_p2_at_dbspl);
}
                       ...
                       float reduction_value)
{
  ...
  float *masking_thresholds = (float*)malloc(sizeof(float) * (fft_size_2+1));
  //float estimated_clean[fft_size_2+1];
  float *estimated_clean = (float*)malloc(sizeof(float) * (fft_size_2+1));
  ...
  free(masking_thresholds);
  free(estimated_clean);
}
